...
```bat
py packaging\assemble-windows-portable.py --app build\windows-staging\app --runtime build\windows-runtime --output build\release\Hermes-Windows-1.8.3-portable.zip
```

该命令输出 ZIP 的 `sha256`。将它填入下一条命令的 `SHA256_FROM_PREVIOUS_COMMAND` 位置：

```bat
py packaging\build-windows-installer.py --makensis "C:\Program Files (x86)\NSIS\makensis.exe" --portable build\release\Hermes-Windows-1.8.3-portable.zip --portable-sha256 SHA256_FROM_PREVIOUS_COMMAND --output build\release\Hermes-Windows-1.8.3-Setup.exe
py packaging\assemble-source.py --output build\release\Hermes-Windows-1.8.3-source.zip
```
...
```bat
py packaging\verify-release.py --portable build\release\Hermes-Windows-1.8.3-portable.zip --installer build\release\Hermes-Windows-1.8.3-Setup.exe --sevenzip "C:\Program Files\7-Zip\7z.exe" --report build\release\verification.json
```

本轮发布采用上面的 NSIS 流程。旧的 Compose `Package-Windows.bat` / MSI 脚本保留用于开发，不是本轮 Setup 的打包入口。历史 `build-startup-repair.py` 仅针对 1.8.0，不能替代 1.8.3 完整安装包。
...
`ConcurrentGatewayTest` 新增新旧确认请求、原始 JSON-RPC id、多问题恢复、英文空格与跨会话取消的回归案例。`Release183Test` 检查草稿附件字节、待确认状态和请求持久化。

在已解锁的桌面执行 `gradlew.bat writeVerificationClasspath`，再运行下列命令可验证真实鼠标与键盘操作：

```powershell
$check183Classpath = Get-Content -Raw build/verification180/classpath.txt
& "$env:JAVA_HOME/bin/java.exe" -cp $check183Classpath com.qingyu.hermescompanion.desktop.Native183Check
```

该检查使用临时演示配置与测试图片，不连接真实网关；会打开实际窗口、模拟鼠标与键盘。截图保存到 `docs/previews/windows-1.8.3`。检查内容包含浮窗与主窗口图片预览、独立删除、关闭预览后草稿保留、主窗口隐藏时 Esc 取消截图、右键取消与框选完成，以及新版确认面板与阴影截图。
